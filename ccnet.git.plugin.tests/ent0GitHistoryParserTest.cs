...
{
	[TestFixture]
	public class Ent0GitHistoryParserTest
	{
		...
		private readonly Ent0GitHistoryParser git = new Ent0GitHistoryParser();

		[Test]
		public void ParsingEmptyLogProducesNoModifications()
		{
			Modification[] modifications = git.Parse(new StringReader(string.Empty), 123);
		...
		{
			Modification[] modifications = git.Parse(new StringReader(oneEntryLog), 123);
			Assert.AreEqual(1, modifications.Length, "#B1");

			Modification mod = modifications[0];
			Assert.AreEqual("123", mod.ChangeNumber, "#B2");
		...
		{
			Modification[] modifications = git.Parse(new StringReader(rubyOnRailsLog), 123);
			Assert.AreEqual(129, modifications.Length, "#E1");

			Modification mod = modifications[0];
			Assert.AreEqual("111", mod.ChangeNumber, "#E2");
			...
			Assert.AreEqual("113", mod.ChangeNumber, "#E13");
